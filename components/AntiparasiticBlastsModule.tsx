...
const AntiparasiticBlastsModule = () => {
    const { userData } = useAuth();
    const [inputText, setInputText] = useState('');
    const [records, setRecords] = useState<ParasiticRecord[]>([]);
    ...
    // Webhook de ANTIPARASITÁRIOS
    const webhookURL = "https://n8n-n8n.w2dtoj.easypanel.host/webhook/11de715b-6fc0-456f-aac7-da41d6b8067c";
    ...
    const handleProcess = () => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const [Data, Cliente, CodCliente, Telefones, Animal, CodAnimal, Anti, Aplicacao] =
            ...
            if (phones.length === 0) {
                out.push({
                    ...
                    Animal, CodAnimal, Antiparasitario: Anti, Aplicacao, valid: false, note: "sem telefone"
                });
                continue;
            }
            ...
            for (const p of phones) {
                ...
                out.push({
                    ...
                    Animal, CodAnimal, Antiparasitario: Anti, Aplicacao, valid: norm.ok, note: norm.ok ? "ok" : norm.reason
                });
            }
        }
    ...
    const buildPayload = () => {
        ...
        const results = valid.map(r => ({
            ...
            antiparasitario: r.Antiparasitario,
    ...
    const handleDownload = () => {
        ...
        a.href = url;
        a.download = `antiparasitarios_${new Date().getTime()}.json`;
        a.click();
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            try {
                ...
                console.error("Error logging parasitic blasts:", logErr);
            }
    ...
    return (
        <div className="fade-in module-container">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                <div>
                    <h1 className="title" style={{ marginBottom: '0.2rem' }}>Processar Planilha de Antiparasitários</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: '2rem', height: '100%', alignItems: 'start' }}>
                ...
                <div className="glass-card" style={{ padding: '1.5rem', height: '100%', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                        Cole aqui as colunas (Data, Cliente, Cód., Tel., Animal, Cód. Animal, Antiparasitário, Aplicação)
                    </p>
                    ...
                    <textarea
                        ...
                        placeholder="Ex: 25/01/2026	João Silva	123	(91) 98888-7777	Rex	456	Bravecto	Programada"
                        value={inputText}
                    ...
                    <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'rgba(67, 24, 255, 0.05)', borderRadius: '12px', border: '1px solid rgba(67, 24, 255, 0.1)' }}>
                        ...
                        <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.75rem', color: 'var(--text-secondary)', lineHeight: '1.6' }}>
                            ...
                            <li>O sistema criará uma linha para cada telefone encontrado.</li>
                            <li>Verifique se a ordem das colunas bate com o exemplo.</li>
                        </ul>
                    </div>
                </div>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                                <thead>
                                    <tr style={{ background: 'var(--bg-color)', borderBottom: '1px solid var(--border-color)' }}>
                                        ...
                                        <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 'bold' }}>Animal/Anti</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {records.length > 0 ? (
                                        records.map((r, i) => (
                                            <tr key={i} style={{ borderBottom: '1px solid var(--border-color)', background: r.valid ? 'transparent' : 'rgba(239, 68, 68, 0.03)' }}>
                                                ...
                                                <td style={{ padding: '12px 16px' }}>
                                                    <div style={{ fontWeight: 600 }}>{r.Animal}</div>
...
export default AntiparasiticBlastsModule;
